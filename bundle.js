...
function getVerticalBounds(data) {
  var _data$reduce = data.reduce(function (returnObj, _ref) {
    var games = _ref.games;

    for (var region in games) {
      var _games$region$reduce = games[region].reduce(function (_ref2, mmr, i) {
        var regionMin = _ref2.regionMin,
            regionMax = _ref2.regionMax;
        ...
        if (i === 0) {
          ...
          regionMin = _games$region$reduce.regionMin,
          regionMax = _games$region$reduce.regionMax;
  ...
  var maxAdj = Math.ceil(max / 500) * 500;
  var minAdj = Math.floor(min / 500) * 500;
  return {
    vmax: maxAdj,
    vmin: minAdj,
...
function getTimeFromTimestamp(timestamp, timeframe) {
  ...
  var lastValidWeekDate = now.getTime() - msInDay * 7;
  var lastValidDayDate = now.getTime() - msInDay;

  var weekDiffInMs = date - lastValidWeekDate;
  var dayDiffInMs = date - lastValidDayDate;

  var weekValid = weekDiffInMs > 0;
  var dayValid = dayDiffInMs > 0;

  var weekMinutesScaled = date.getMinutes() / 60 * 0.1;
  var weekHoursScaled = date.getHours() / 24;
  var dayMinutesScaled = date.getMinutes() / 60;
  return {
...
exports.getHorizontalBounds = getHorizontalBounds;
exports.mix = mix;
...
var _require = __webpack_require__(7),
    getMatches = _require.getMatches;

var _require2 = __webpack_require__(0),
    getVerticalBounds = _require2.getVerticalBounds,
    getHorizontalBounds = _require2.getHorizontalBounds;

var _require3 = __webpack_require__(2),
    backgroundColor = _require3.backgroundColor,
    textColor = _require3.textColor;

var _require4 = __webpack_require__(0),
    getTimeFromTimestamp = _require4.getTimeFromTimestamp,
    mix = _require4.mix;

var sortedUniqBy = __webpack_require__(8);
var regions = __webpack_require__(3);

/* Charts */

var _require5 = __webpack_require__(11),
    background = _require5.background;

var _require6 = __webpack_require__(12),
    avgline = _require6.avgline;

var _require7 = __webpack_require__(13),
    scatter = _require7.scatter;

var _require8 = __webpack_require__(14),
    divider = _require8.divider;
...
// Time selector
var timeframeDiv = document.createElement('div');
applyHeaderStyle(timeframeDiv);
var timeframeSelector = document.createElement('select');
...
timeframeSelector.onchange = function (e) {
  drawGraphs();
};
...
var _loop = function _loop(region) {
  /* Button */
  var name = regions[region].name;

  var regionDiv = document.createElement('div');
  regionDiv.id = name;
  regionDiv.onclick = function (ev) {
  ...
  /* Text */
  var regionTextDiv = document.createElement('div');
  applyTextStyle(regionDiv);
  regionDiv.appendChild(regionTextDiv);
  var regionText = document.createTextNode(name);
...
infoModal.appendChild(__webpack_require__(24));
...
infoImage.src = __webpack_require__(25);
...
function styleHeader() {
  for (var region in regions) {
    var _regions$region = regions[region],
        lineColor = _regions$region.lineColor,
        enabled = _regions$region.enabled,
        name = _regions$region.name;

    var regionDiv = document.getElementById(name);
...
getMatches(function (err, res) {
  if (err) {
    ...
    var errorMessage = document.createTextNode(err);
    ...
    data = res.map(function (e) {
      ...
      for (var region in e.games) {
        e.games[region] = e.games[region].filter(function (e) {
          return e > 5500;
        });
      }
      return e;
...
var endpoint = 'https://5zemss3woc.execute-api.us-east-1.amazonaws.com/prod/matches';
  ...
  xhttp.onreadystatechange = function () {
    if (this.readyState === 4 && this.status === 200) {
      try {
        ...
        var data = items.map(function (match) {
          var date = match.gamesDate.N;
          var games = JSON.parse(match.games.S);
          return {
            date: date,
            games: games
          };
...
var Chartist = __webpack_require__(1);

function background(data, options, graphDiv) {
  var _options$verticalBoun = options.verticalBounds,
      vmin = _options$verticalBoun.vmin,
      vmax = _options$verticalBoun.vmax,
      vsteps = _options$verticalBoun.vsteps;
  var _options$horizontalBo = options.horizontalBounds,
      hmin = _options$horizontalBo.hmin,
      hmax = _options$horizontalBo.hmax,
      hsteps = _options$horizontalBo.hsteps;
  ...
  var bg = Chartist.Line('#chart-background',
  // Data
  {},
  // Options
  {
...
function formatDataLineGraph(data, timeframe) {
  ...
  var seriesData = data.filter(function (stats) {
    if (stats.time[timeframe] === null) return false;
    return true;
  }).filter(function (e, i) {
    return i % toTake === 0;
  }).reduce(function (regionData, stats) {
    var _loop = function _loop(region) {
      var regionInfo = regions[region];
      if (!regionInfo || regionInfo.enabled === false) return 'continue';
      var games = stats.games[region];
      var i = regionInfo.index;
      if (regionData[i] === undefined) regionData[i] = [];

      var sumCount = 0;
      var numToRemove = Math.floor(games.length * 0.2);
      var sum = games.sort(function (a, b) {
        return b - a;
      }).slice(numToRemove, games.length - numToRemove).reduce(function (sum, mmr) {
        sumCount++;
        return sum + mmr;
      }, 0);
      var avg = sum / sumCount;
    ...
    // Series
    for (var region in stats.games) {
      var _ret = _loop(region);

      if (_ret === 'continue') continue;
    }
    return regionData;
  }, []);
  var sortedSeriesData = seriesData.map(function (series) {
    return series.sort(function (a, b) {
      return a.x - b.x;
    });
  });
  return { series: sortedSeriesData.filter(function (e) {
      return e !== undefined;
    }) };
}
...
function avgline(data, options, graphDiv) {
  ...
  var lineData = formatDataLineGraph(data, options.timeframe);
  var lineChart = new Chartist.Line('#avg-line-chart', lineData, {
    chartPadding: {
      right: 40
    },
    showPoint: false,
  ...
  lineChart.on('draw', function (context) {
    if (context.type === 'line') {
      var styles = ['stroke: ' + context.series[0].meta, 'stroke-width: 4px'];
...
var _require = __webpack_require__(0),
    getOpacity = _require.getOpacity,
    mix = _require.mix;

var _require2 = __webpack_require__(2),
    backgroundColor = _require2.backgroundColor;

function formatDataScatterPlot(data, timeframe) {
  var seriesDataObj = data.filter(function (stats) {
    if (stats.time[timeframe] === null) return false;
    return true;
  }).reduce(function (regionData, stats) {
    var then = new Date(Number(stats.date));
    ...
    var _loop = function _loop(region) {
      ...
      regionData[region] = regionData[region].concat(games.map(function (mmr) {
        return {
          x: stats.time[timeframe],
          y: mmr,
  ...
  var seriesData = [];
  for (var key in seriesDataObj) {
    if (seriesDataObj.hasOwnProperty(key)) {
      seriesData.push(seriesDataObj[key]);
    }
  }
  return { series: seriesData.filter(function (e) {
      return e !== undefined;
    }) };
}
...
function scatter(data, options, graphDiv) {
  ...
  var scatterData = formatDataScatterPlot(data, options.timeframe);
  var scatterPlot = new Chartist.Line('#scatter-plot',
  // Data
  scatterData,
  // Options
  {
  ...
  scatterPlot.on('draw', function (context) {
    if (context.type === 'point') {
      var styles = ['stroke: ' + context.meta, 'stroke-width: 4px', 'stroke-linecap: round'];
      context.element.attr({
        style: styles.join('; ')
      });
    }
...
var _require = __webpack_require__(0),
    getTimeFromTimestamp = _require.getTimeFromTimestamp;

var _require2 = __webpack_require__(2),
    dividerColor = _require2.dividerColor;

function formatDataDividerGraph(data, min, max, timeframe) {
  var now = new Date();
  var time = getTimeFromTimestamp(now.getTime());
  return { series: [[{
      x: time[timeframe],
      y: min
    }, {
...
function divider(data, options, graphDiv) {
  ...
  var lineData = formatDataDividerGraph(data, vmin, vmax, options.timeframe);
  var lineChart = new Chartist.Line('#divider', lineData, {
    chartPadding: {
      right: 40
    },
    showPoint: false,
  ...
  lineChart.on('draw', function (context) {
    if (context.type === 'line') {
      var styles = ['stroke: ' + dividerColor, 'stroke-width: 3px', 'stroke-dasharray: 25px 8px;'];
      context.element.attr({
        style: styles.join('; ')
      });
    }
...
if(false) {
	...
	if(!content.locals) {
		module.hot.accept("!!../../css-loader/index.js!./chartist.css", function() {
			var newContent = require("!!../../css-loader/index.js!./chartist.css");
...
var titleText = document.createTextNode('Dota Queue Monitor');
title.appendChild(titleText);
infoModalContent.appendChild(title);
textArr.forEach(function (node) {
...
module.exports = __webpack_require__.p + "fonts/info.svg";
